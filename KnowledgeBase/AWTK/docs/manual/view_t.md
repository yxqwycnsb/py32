...
view_t是[widget_t](widget_t.html)的子类控件，widget_t的函数均适用于view_t控件。
...
> 缺省获得焦点的子控件(可用控件名或类型)。
> 
> view作为pages/slideview的直接子控件才需要设置。 正常情况下，一个窗口只能指定一个初始焦点。 但是对于pages/slideview来说，可能希望每一个页面都有一个初始焦点，此时可用default_focused_child来指定。
