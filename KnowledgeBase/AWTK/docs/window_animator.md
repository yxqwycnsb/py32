...
> 
> 对话框的动画可与 [对话框的 highlight 属性](dialog_highlight.html) 一起使用。
...
> anim_hint 的可选值，请参考 [window_animator.h](https://github.com/zlgopen/awtk/blob/master/src/base/window_animator.h) 中的定义。

## 三、动画参数

可以为动画指定参数，其格式为 [类似函数调用的参数格式](func_call_params_format.html)。

  1. 所有动画都支持下面的参数：


  * duration 动画持续时间（毫秒）
  * easing [插值算法名称](easing.html)
