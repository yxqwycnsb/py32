...
函数名称 | 说明  
---|---  
conf_ini_create | 创建一个空的conf对象。  
conf_ini_load | 从指定URL加载INI对象。  
conf_ini_load_from_buff | 从内存加载INI对象。  
conf_ini_save_as | 将doc对象保存到指定URL。  
conf_ini_save_to_buff | 将obj保存为INI格式到内存。  
