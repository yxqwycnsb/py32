...
dialog_title_t是[widget_t](widget_t.html)的子类控件，widget_t的函数均适用于dialog_title_t控件。

在xml中使用”dialog_title”标签创建dialog_title。如：
...
## 在c代码中，用dialog_create_simple创建对话框时，自动创建dialog标题对象。
...
参数 | 类型 | 说明  
---|---|---  
返回值 | widget_t* | dialog_title对象。  
widget | widget_t* | dialog_title对象。  
  
#### dialog_title_create 函数
...
#### dialog_title_get_widget_vtable 函数
...
参数 | 类型 | 说明  
---|---|---  
返回值 | const widget_vtable_t* | 成功返回 dialog_title 虚表。
