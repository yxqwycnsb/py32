...
SDL 的 Texture 为 SDL_PIXELFORMAT_RGBA8888 时，picasso 的 canvas 的格式要使用 COLOR_FORMAT_ABGR。
...
在 picasso 的 canvas 格式使用 COLOR_FORMAT_ABGR 时，其图片采样格式与 stb 加载的格式相反。所以修改了 gfx_span_image_filters.h，让 RGBA 的图片按固定格式采样。
