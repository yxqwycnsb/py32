...
[FScript](fscript.html) 的 widget 扩展函数只能在 widget 的事件处理函数中使用，包括传统的 AWTK 应用程序和 AWTK-MVVM 应用程序的事件处理函数。
...
> 打开窗口。 相关函数：window_open、window_close_and_open。
...
> 退到主窗口 相关函数：window_back_to_home。
...
> 向指定控件发生按键事件 相关函数：widget_send_key。
...
> 获取本地化信息(国家和语言) 相关函数：widget_locale_get。
...
> 设置本地化信息(国家和语言) 相关函数：widget_locale_set。
...
> 设置本地化信息(国家和语言) 相关函数：widget_theme_set。
