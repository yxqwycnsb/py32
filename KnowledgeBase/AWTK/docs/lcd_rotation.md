...
AWTK 内部还提供了快速旋转模式，该机制能在旋转时保持较高的效率（和没有旋转时的运行效率几乎一样），但是兼容性比较差，具体如何使用请参考 [如何使用高效的屏幕旋转](https://github.com/zlgopen/awtk/tree/master/docs/how_to_use_fast_lcd_portrait.html)

## 2 不同 LCD 实现方式对旋转的支持

AWTK 目前支持 3 种 LCD 实现方式，不同的实现方式对旋转的支持有所不同，下面我们一一介绍。

### 2.1 基于寄存器的 LCD(lcd_reg)
...
### 2.2 基于 FrameBuffer 的 LCD(lcd_mem)

  1. 对触摸事件的坐标进行转换。
  2. 设置窗口管理器的大小为旋转之后的大小。
  3. offline fb 为旋转之后的大小，online fb 为原始大小。在 lcd flush 时需要进行旋转（在没有硬件旋转减速的情况下，会增加一点性能开销）。
  4. 在进行平移窗口动画时。AWTK 将图片直接贴到 online fb 上，处于性能的考虑，在截图时进行旋转，在贴图时直接拷贝。所以在 lcd_mem_draw_image 和 lcd_mem_take_snapshot 两个函数中做了处理。



### 2.3 基于 VGCanvas 的 LCD(lcd_vgcanvas)
