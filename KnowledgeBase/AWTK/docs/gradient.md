...
需要注意的是，目前 AWTK 只支持将控件风格（样式）中的 bg_color 属性和 fg_color 属性设置为渐变色，即只允许控件背景色和前景色为渐变色。
...
在 C 代码中设置控件背景色为渐变色只需调用 widget_set_style_str 接口即可，代码如下：
    
    
    widget_set_style_str(button, "normal:bg_color", "linear-gradient(180deg, red, blue);");
    

> 备注：调用 widget_set_style_xxx 接口在代码中设置的样式为 inline style，会占用额外的内存，具体详见 [theme.md](theme.html)。
